...
const InventoryAuditTrail: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const data = await getInventoryAuditTrail(
        ...
        filters.itemName, filters.category, filters.actionType
      );
      setAuditData(data);
    } catch (err: any) {
  ...
  const exportToPDF = () => {
    ...
    const headers = [['Item Name', 'Category', 'Quantity Added', 'Date/Time', 'Action', 'Previous Stock', 'New Stock']];
    const rows = auditData.map(record => [
      record.item_name, record.category, record.quantity_added.toString(),
      formatDateTime(record.date_time), record.action_type,
      record.previous_stock.toString(), record.new_stock.toString()
    ]);
  ...
  if (!user) return <div className="p-6">Please login to view inventory audit trail.</div>;
    ...
    <div className="space-y-6">
      <div className="card p-6">
        ...
        <p className="text-gray-600 mb-6">
          Read-only log of all inventory changes. To correct entries, use <strong>Stock Update → Recent Inventory Uploads</strong>.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          <div>
            ...
            <input type="date" className="w-full" value={filters.startDate} onChange={e => handleFilterChange('startDate', e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">End Date</label>
            <input type="date" className="w-full" value={filters.endDate} onChange={e => handleFilterChange('endDate', e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Item Name</label>
            <input type="text" className="w-full" placeholder="Search item name" value={filters.itemName} onChange={e => handleFilterChange('itemName', e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Category</label>
            <select className="w-full" value={filters.category} onChange={e => handleFilterChange('category', e.target.value)}>
          ...
          <div>
            ...
            <select className="w-full" value={filters.actionType} onChange={e => handleFilterChange('actionType', e.target.value)}>
              <option value="">All Actions</option>
              {actionTypes.map(action => <option key={action} value={action}>{action.replace('_', ' ')}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={exportToExcel} disabled={loading || auditData.length === 0} className="btn btn-secondary px-4 py-2 text-sm disabled:opacity-50">Export to Excel</button>
          <button onClick={exportToPDF} disabled={loading || auditData.length === 0} className="btn btn-secondary px-4 py-2 text-sm disabled:opacity-50">Export to PDF</button>
          <button onClick={fetchData} disabled={loading} className="btn btn-primary px-4 py-2 text-sm disabled:opacity-50">Refresh</button>
        </div>
        ...
        {auditData.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{auditData.filter(r => r.action_type === 'ADD_ITEM').length}</div>
              <div className="text-sm text-green-800">Items Added</div>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{auditData.filter(r => r.action_type === 'PURCHASE').length}</div>
              <div className="text-sm text-purple-800">Purchases</div>
            </div>
            <div className="bg-orange-50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">{auditData.reduce((sum, r) => sum + r.quantity_added, 0)}</div>
        ...
        {/* Data Table */}
        {loading ? (
          <div className="text-center py-8"><p className="text-gray-500">Loading audit trail...</p></div>
        ) : error ? (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4"><p className="text-red-700">Error: {error}</p></div>
        ) : auditData.length === 0 ? (
          <div className="text-center py-8"><p className="text-gray-500">No audit records found.</p></div>
        ) : (
                ...
                {auditData.map((record) => (
                  <tr key={record.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4">
                      <span className="px-2 py-1 bg-gray-100 text-gray-800 text-xs rounded">{record.category}</span>
                    </td>
                    ...
                    <td className="py-2 pr-4 text-right font-semibold text-green-600">+{record.quantity_added}</td>
                    ...
                    <td className="py-2 pr-4 text-gray-600 max-w-xs truncate">{record.notes || 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
